...
func pursuit_score(player: BaseballPlayer) -> float:
	...
	var arrival := player.position.distance_to(read.target) / player.data.speed + player.reaction_remaining
	...
	return maxf(arrival, read.playable_in) + departure_cost * nearby_weight - (0.25 if player == chaser else 0.0)
...
func _reassign(initial: bool) -> void:
	...
	var arrival := chaser.position.distance_to(point) / chaser.data.speed + chaser.reaction_remaining
	var recovering: bool = game.play != null and game.play.catch_retries.get(chaser, 0.0) > 0.0
	var covered: bool = arrival < 0.4 or (not game.ball.bounced and arrival + 0.2 < read.playable_in and chaser.data.anticipation >= 0.55)
	backup = null
...
func nearest(point: Vector2, excluded: Array) -> BaseballPlayer:
	...
	for player in fielders:
		if player in excluded:
			continue
		var arrival := player.position.distance_to(point) / player.data.speed + player.data.reaction_time
