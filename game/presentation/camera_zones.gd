...
func _draw() -> void:
	...
	draw_string(ThemeDB.fallback_font, Vector2(-500, y + 25), "Bottom of camera view", HORIZONTAL_ALIGNMENT_LEFT, -1, 20, Color(1, 0.35, 0.35))
...
func _draw_zone(rect: Rect2, color: Color, label: String) -> void:
	...
	draw_string(ThemeDB.fallback_font, local_rect.position + Vector2(8, -8), label, HORIZONTAL_ALIGNMENT_LEFT, -1, 20, color)
