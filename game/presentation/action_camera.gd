...
func _process(delta: float) -> void:
	...
	var runner: BaseballPlayer = (
		active_runner.runner if active_runner != null else match_scene.batter
	)
	...
	var overview_zoom := minf(
		close_zoom, minf(view_size.y / (depth + 240.0), view_size.x / (depth * sqrt(2.0) + 160.0))
	)
	var focus := overview
	...
	if (
		phase
		in [
			match_scene.Phase.FIELDING,
			match_scene.Phase.THROW,
			match_scene.Phase.TAG,
			match_scene.Phase.HOME_RUN,
			match_scene.Phase.FOUL
		]
	):
	...
	desired_zoom = minf(
		desired_zoom, minf(view_size.x / protected.size.x, view_size.y / protected.size.y)
	)
	if match_scene.dugout_view:
		var benches := (
			Rect2(match_scene.dugouts[0].position, Vector2.ZERO)
			. expand(match_scene.dugouts[1].position)
			. grow(280.0)
		)
...
func _update_inset(runner: BaseballPlayer, delta: float) -> void:
	...
	var in_view := Rect2(Vector2(60, 95), viewport_size - Vector2(120, 155)).has_point(
		screen_position
	)
	...
	if not active:
		...
		if visible_time > 0.35:
			inset.hide()
	runner_view.render_target_update_mode = (
		SubViewport.UPDATE_ALWAYS if inset.visible else SubViewport.UPDATE_DISABLED
	)
	...
	runner_view.canvas_transform = Transform2D(
		Vector2(scale_factor, 0), Vector2(0, scale_factor), size / 2.0 - focus * scale_factor
	)
	inset_label.text = (
		"%s → %s" % [runner.data.player_name, BaseballBaseRunning.base_name(running.target_base)]
	)
