...
func _ready() -> void:
	...
	for index in 17:
		...
		boundary.append(Vector2(-flat_half_width, top + corner_radius) + Vector2.from_angle(angle) * corner_radius)
	for index in 17:
	...
	for side in [-1, 1]:
		...
		line.polygon = PackedVector2Array([home_position - offset, end - offset, end + offset, home_position + offset])
	...
	for index in boundary.size() - 1:
		...
		if a >= -0.01 and b < 0.0 and a - b > 0.0001:
			...
			if fraction < earliest:
				earliest = fraction
				hit = {"point": start.lerp(finish, fraction), "normal": normal, "fraction": fraction}
	return hit
	...
	while remaining > 0.0001:
		...
		if not hit.is_empty():
			...
			if not bounced and crossing_height > fence_height and is_fair(hit.point):
				samples.append({"point": clamp_inside(hit.point), "height": height, "home_run": true, "time": seconds - remaining + delta})
				return samples
			point = hit.point + hit.normal * 2.0
			velocity = velocity.bounce(hit.normal) * 0.45
			bounced = true
		remaining -= delta
		samples.append({"point": point, "height": height, "home_run": false, "time": seconds - remaining})
	return samples
